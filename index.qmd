...
<!-- TODO thin this out -->
It's not just about convenience, there are also practical considerations. At the time, agent harnesses would load all MCP tool definitions are loaded up front, which takes up a bunch of tokens. (That's gotten better since then, harnesses like Claude Code have gotten smarter about this.) But particularly with the smaller size of context windows that models had at the time, that was a real concern.
...
:::{.notes}
Plus, there are other tradeoffs that come with the convenience of using an existing CLI.

<!-- TODO Thin this out -->
Something like `gh` that can communicate to a remote service, sure it can read issues, but it can also act on your behalf with anything else that you can do in GitHub. It is very difficult to allow the agent to do some things with `gh` but prevent it from doing other things. Or rather, you can do a reasonable job of managing it with allow lists and PreToolHooks and such, but what is your risk tolerance? There are plenty of examples on the internet of agents going rogue and deleting the production database and whatnot. If the tool is available, an agent may be able to get at it.
...
In particular, I would be really mindful of creating tools that allow writes or deletes. Not that you can't do it, but they may not be safe in all contexts: it may be fine if this is the only MCP server your agent has, but not in combination with other tools. 
...
:::{.notes}
It's just an API. A standard, meant
for agents rather than for human hands.
You write the functions and declare intent;
the model reads those tools and understands

the rest. So write your own, and by deploying
it, give your stalled-out workflows a new spine;
an agent that stood helpless starts employing
your superpowers. And you draw the line

where it should stop. That line is your protection,
