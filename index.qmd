...
Aside from the convenience of that, there was a practical consideration. At the time, agent harnesses would load all MCP tool definitions are loaded up front, which takes up a bunch of tokens. Particularly with the smaller size of context windows that models had at the time, that was a real concern.
...
This is the exact case of the `gh` command line utility I mentioned. But the advantages of a CLI over MCP are weaker if the CLI is something new you're writing: it's not in the training data, so you have to teach the agent how to use it--that's context you have to take up. By the time your agent has "learned" how to use your new CLI, you've consumed all of the context savings you were hoping to have. Plus, if it is a new CLI you have written, you have a distribution problem, you have to get everyone to install it.
:::
...
Now, we can't solve this with a well designed MCP server, ...
:::

## We can make good choices when we write our own tools {.center}

:::{.notes}
... because it's about the agent context where you're using an MCP or CLI, and it may be only in the combination of multiple tools, outside of a single MCP or CLI, that the trifecta is met. But if we keep this in mind when designing our MCP servers, we can mitigate some risks. 
