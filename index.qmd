...
A key point I want to convey, and that underlies much of the talk, is that:
:::
...
There are many standards for APIs out there, you have REST APIs, JSON RPC, many others, and each API defines its conventions for how you communicate with it. These conventions are designed to make it easy to do certain types of work with the API, the things they were designed to do. MCP is just another standard for how to make APIs, tailored to the needs of AI applications.
...
Now, we can't solve this with a well designed MCP server, because it's about the agent context where you're using an MCP or CLI, and it may be only in the combination of multiple tools, outside of a single MCP or CLI, that the trifecta is met. But...
:::
...
In particular, I would be really mindful of creating tools that allow writes or deletes. Not that you can't do it, but maybe you don't want them generally available to all agents using your MCP server. But since we're talking about writing your own, you can freely write multiple servers that have different combinations of tools, for use in different agents, if you want.
...
You write a function and declare intent;
the model reads that line and understands

the rest. So write your own, and by deploying
it, give the stalled-out workflow a new spine;
the agent that stood helpless starts employing
your superpowers. And you draw the line

where it should stop. Some risk is still inherent:
a model cannot tell text from commands,
and won't. But build a server that's coherent
and narrow. Keep your keys out of its hands.

You'll write one that does just what you expect,
then hand the hosting over to Connect.
:::
