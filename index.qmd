...
## {background-image="img/diptych-1.jpeg" background-size="contain"}
...
Posit's enterprise products run on-premises, which means that if something goes wrong, the engineers can't see what's going on directly because we don't have access to the servers and networks where they're running.
:::

## {background-image="img/diptych-2.jpeg" background-size="contain"}
...
## {background-image="img/balthasar-1.jpeg" background-size="contain"}

::: {.notes}

Posit uses Zendesk for support, customers can file tickets there and they get fielded by our support team. And there's usually some back and forth with followup questions and requests for details before it escalates to the engineers. 
:::

## {background-image="img/balthasar-2.jpeg" background-size="contain"}

::: {.notes}

So by the time the engineers get it, the ticket can be quite long, and there can be large attachments with diagnostic data in them in addition to all of the messages. 
...
(don't say Connect on this slide)

A key point that underlies much of the talk is that:
:::
...
There are many standards for APIs out there, you have REST APIs, JSON RPC, many others, and each API defines its conventions for how you communicate with it. MCP is just another standard for how to make APIs, tailored to the needs of AI applications.

This realization helped demystify it for me. As someone who has worked in software and data for many years, I know how to use APIs, I know how to build and serve them too. 
:::

## {background-image="img/unix-system-old.jpeg" background-size="contain"}
...
:::{.notes}
Local MCPs, where the code is running on your machine, potentially downloaded from anywhere really, and...
:::
...
## {background-image="img/distracted-boyfriend-old.jpeg" background-size="contain"}
...
It's not just about convenience, there are also practical considerations. MCP tool definitions consume tokens in your context window, and while harnesses like Claude Code have gotten smarter about deferring some of this, there is some overhead. And particularly with the smaller size of context windows that models had at the time, that was a real concern.
...
::: {.notes}
Of course, the truth isn't so black and white. So let's walk through the options and tradeoffs. There are three alternatives I want to consider here: MCP, CLIs, and Skills. Let's start with Skills.
:::
...
:::{.notes}
Sometimes the thing you need is just additional context or instructions for the agent, not new tools. That's where you want a Skill. 
:::
...
:::{.notes}
<!-- TODO: this one was a little ragged -->
The strongest case for using a CLI over MCP, particularly for the case I'm interested in here where we're trying to connect a remote service to my agent, is when...

This is the exact case of the `gh` command line utility I mentioned. You save on lots of context because you don't even need a Skill to explain how to use it, the model has already been trained on it.

But the advantages of a CLI over MCP are weaker if the CLI is something new you're writing: it's not in the training data, so you have to teach the agent how to use it--that's context you have to take up. By the time your agent has "learned" how to use your new CLI, you've consumed all of the context savings you were hoping to have. Plus, if it is a new CLI you have written, you have a distribution problem, you have to get everyone to install it.
:::
...
:::{.notes}
Plus, there are other tradeoffs that come with the convenience of using an existing CLI, which typically was not designed for use in AI.

Something like `gh` that can communicate to a remote service, sure it can read issues, but it can also act on your behalf with anything else that you can do in GitHub. Once your agent can run stuff in `bash`, it is difficult to allow it to do run certain CLI commands but prevent it, with certainty, from running others. 

To be clear, I use `gh` in Claude Code on my laptop with "auto mode", and I feel good about that, but there are other more autonomous agentic contexts where I want to be stricter than that. Because there are plenty of examples of agents going rogue and deleting the production database and whatnot. If a tool is available, an agent may be able to invoke it. You can do a reasonable job of managing what CLI commands it can run with allow lists and PreToolHooks and such, but what is your risk tolerance?

Now that I've raised the prospect of rogue agents and our risk tolerance, we need to talk a bit more about security and AI.
:::

## Act IV: Something Wicked This Way Comes {.center}

:::{.notes}
Agentic AI can be so valuable for us because it can, so to speak, "be creative" with how it uses the tools available to it to do tasks we want it to. But that same power, that same creativity, brings risks, because it could also do destructive things, whether on accident while trying to creatively solve our problem---or because someone else has given it different directions. 

One framing that is useful for thinking about this risk is the "lethal trifecta":
:::
...
It's disturbingly easy to get this. Let's assume that you're working with private data with your agent.
Suppose you're reading GitHub issues on a public repo, whether with GitHub MCP or the `gh` CLI. Someone could write a prompt injection into an issue, and that gives you the second of the three of the trifecta. Even just the ability to make an arbitrary HTTP request using WebFetch is enough to satisfy the trifecta.

Now, we can't solve this with a well designed MCP server or CLI, because it's about the agent context where you're using it, and it may be only in the combination of multiple tools, outside of a single MCP or CLI, that the trifecta is met. But...
:::

## We can make good choices when we write our own tools {.center}

:::{.notes}
But if we keep this in mind when designing our MCP servers, we can mitigate some risks. 

How should we do that? How can we get the best results, and how do we avoid some of the bad things that can happen when agents go rogue?
...
Another reason is security: the best way to prevent an agent from calling a dangerous tool is to never give it the tool at all. 

In particular, I would be really mindful of creating tools that allow writes or deletes. Not that you shouldn't ever do it, but think about how the lethal trifecta works. It's about the full agent context, not any single set of tools. So tools that can do writes may be fine if this is the only MCP server your agent has, but not in combination with other tools. 

But since we're talking about writing your own, you can freely write multiple servers that have different combinations of tools, for use in different agents, if you want.
:::
...
Everything the MCP tool returns goes into the context. So if we're building something in front of an external service's API, we need to strip out the details that it returns that aren't useful for our purposes. Extraneous stuff just burns more tokens and can confuse the model.
:::
...
So, these are some things to keep in mind when writing your own. And...
:::
...
::: {.notes}
If you're going to write your own, unless the purpose of the tool is to manipulate things on your local file system, make it a remote MCP server that you host. It works everywhere, doesn't have to execute code on your machine, and on a hosting platform like Posit Connect, you get lots of nice support, like authentication, which is built into the MCP standard. 

---

Ok. I want to wrap up by putting this all together. On the title side, I said this play was a comedy, not a tragedy. So, despite the lethal trifecta, everyone doesn't die, and we do have a happy ending.
:::
...
Now, we're going to deploy this to Posit Connect, just as we do any other kind of app or API that we host there. And the first thing that Connect is going to solve for us is authentication to Zendesk. If we ran this MCP server locally, me and everyone else who wanted to use it would need to get an API key for Zendesk and manage their credentials somehow on their local machines. That's a bit of a burden, not to mention a risk that my Security team would not allow. 
:::
...
Ok, so I copied that command and added it to my Claude Code. Notice that there is no API key in that command, so when I start Claude Code, I need to authenticate.
:::
...
:::{.notes}
This is the next place that Posit Connect really helps us because it supports that OAuth flow I mentioned before. So we open up the MCP tools, select "Authenticate", and it opens up a web browser for us to authenticate to Connect. When it's done it automatically closes the browser and returns you here.
:::
...
:::{.notes}
Shall I compare thee to an API?
Thou art but that, no more, and nothing grand:
mere plumbing that is built to serve AI.
The agents read your tools and understand

the rest. So write your own, and then deploy
it, let your whole team use what you design;
and free the locked-out agents to employ
your superpowers. And you draw the line

where they should stop. That line is your protection,
for models can't distinguish your commands
from text a stranger wrote. That's prompt injection:
so narrow tools leave it with empty hands.

Your server does just what you do direct,
so freely build. The rest is for Connect.
:::
